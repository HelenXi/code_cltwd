...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 18 10:58:19 2022
...
def generate_uniform_sphere(d,n):
    ...
    for j in range(n):
        ...
        data[j] = temp/np.linalg.norm(temp) + np.array([3*np.random.choice([-1,1]),0,0])
    return data
...
def generate_uniform_ellipsoid(d,n,sigma,v):
    ...
    for j in range(n):
        ...
        data[j] = temp/np.sqrt(np.linalg.norm(temp)**2 +(1/sigma**2-1)*temp[v]**2) + np.array([3*np.random.choice([-1,1]),0,0])
    return data
...
for li in replacement:
    ...
    for i in range(100):
        ...
        for seed in range(n_seed):
            smp[seed] = ot.sliced.max_sliced_wasserstein_distance(datap, dataq, a, b, 1000)**2
        swd[i] = np.mean(smp)
    datap = generate_uniform_ellipsoid(d,n,sigma,v)
    dataq = generate_uniform_sphere(d,n)
    smp = np.empty((10,))
    ...
    for j in range(B):
        ...
        req = dataq[indices]
        for seed in range(n_seed):
            btp[seed] = ot.sliced.max_sliced_wasserstein_distance(rep, req, al, bl, 1000)**2
